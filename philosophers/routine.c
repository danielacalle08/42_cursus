...
/*   Updated: 2024/08/03 17:02:10 by dcalle-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	eat_routine(t_philo *philosopher)
{
	...
	print_info(COLOR_WHITE, "has taken a fork", philosopher);
	if (philosopher->left_fork == philosopher->right_fork)
	{
		pthread_mutex_unlock(philosopher->right_fork);
		print_info(COLOR_RED, "died", philosopher);
	...
	print_info(COLOR_GREEN, "is eating", philosopher);
...
void	*routine(void *arg)
{
	...
	while (!check_dead(philo))
	{
		...
		if (!philo->data->nbr_meals || (philo->meals < philo->data->nbr_meals))
			...
			break ;
		print_info(COLOR_WHITE, "is sleeping", philo);
		ft_usleep(philo->data->time_to_sleep);
		print_info(COLOR_WHITE, "is thinking", philo);
	}
	return (arg);
}
